...
(
   ...
   ASSIGNMENT            DBMS_SQL.VARCHAR2_TABLE,   
   ROWIDS                rowidArray
);
...
(
   ...
   ASSIGNMENTS           DBMS_SQL.VARCHAR2_TABLE,
   ROWIDS                rowidArray
);
...
PROCEDURE QAP_ICC_REVIEW_CORRECTIONS
(
P638_PID             NUMBER,
P638_DID_QC_PASS     VARCHAR2,
P638_COMMENTS        VARCHAR2,
P638_REVIEWED_BY     VARCHAR2
);

procedure  REMOVE_SQA_ETL_DATA;
...
BEGIN
 ...
 MSG     := 'Starting..';
    ...
    SELECT TO_NUMBER(VARIABLE_VALUE)
     INTO  P_ARCHIVE_DAYS
     FROM  SQA_SYS_VARIABLES
     WHERE VARIABLE_NAME = 'ARCHIVE_DAYS';
...
SEL_STMT := ' SELECT PID, ROWID FROM SQA_TD_DATA WHERE COMPLETED_DT <  :A ';
...
OPEN GC FOR SEL_STMT USING  (TRUNC(SYSDATE) - P_ARCHIVE_DAYS);
...
SQL_STMT := 'SELECT PID, NVL(R.WORKING,''N'')AS WORKING, NVL(R.SQA_TD_REP,''NOT-ASSIGNED'')  AS SQA_TD_REP, R.REVIEW_ID,  0 AS BATCH_NO, R.COMPLETED_DT, R.RK ';
...
SQL_STMT := SQL_STMT||'           where   CONTRACTOR = :A';
SQL_STMT := SQL_STMT||'           AND     REPORT_SEGMENT = :B';
SQL_STMT := SQL_STMT||'           AND     COMPLETED_DT >= :C ) R ';
SQL_STMT := SQL_STMT||' ORDER BY R.RK ';
         ...
         CLOSE GC;
            ...
            FOR i IN 1..TD.PID.COUNT loop               
                ...
                IF (TD.COMPLETED_DT(i) > ( GV_CURRENT_DATE  - 60) AND 
                    TD.WORKING(i)      = 'N'  AND 
                    TD.SQA_TD_REP(i)   = 'NOT-ASSIGNED' ) 
                THEN 
                       TD.WORKING(i)    := 'Y';
...
GV_CURRENT_MINUS   :=  GV_CURRENT_DATE - 7;


CNT := 0;

SQL_STMT := 'SELECT T.SPIPROPERTYID,T.LOANNUMBER,T.LOANTYPE,T.CLIENT,T.ADDRESSLINE1,T.ADDRESSLINE2,T.CITY,T.STATE,T.ZIP,T.ORDERNUMBER,T.ORDERDATE,T.WORKCODE,T.COMPLETEDDATE,T.CONTRACTORCODE,T.MORTGAGORNAME,T.SALEDATE,T.SECUREDDATE,T.WINTERIZEDDATE,';
SQL_STMT := SQL_STMT||' T.INITIALICCDATE,T.LASTICCDATE,T.CONVEYDATE,T.INVESTORNUMBER,T.BILLINGCODE,T.REPORT_SEGMENT,T.WORK_GROUP,T.NBR_QUESTIONS,T.PID';
SQL_STMT := SQL_STMT||' FROM (SELECT A.SPIPROPERTYID,A.LOANNUMBER,A.LOANTYPE,A.CLIENT,A.ADDRESSLINE1,A.ADDRESSLINE2,A.CITY,A.STATE,A.ZIP,A.ORDERNUMBER,A.ORDERDATE,A.WORKCODE,A.COMPLETEDDATE,A.CONTRACTORCODE,A.MORTGAGORNAME,A.SALEDATE,A.SECUREDDATE,A.WINTERIZEDDATE, ';
SQL_STMT := SQL_STMT||'              A.INITIALICCDATE,A.LASTICCDATE,A.CONVEYDATE,A.INVESTORNUMBER,A.BILLINGCODE,A.REPORT_SEGMENT,A.WORK_GROUP,A.NBR_QUESTIONS,A.PID';
SQL_STMT := SQL_STMT||'  FROM SQA_TD_DATA_ETL a';
SQL_STMT := SQL_STMT||'  LEFT JOIN ( SELECT PID FROM SQA_TD_DATA ) B ON (A.PID = B.PID)';
SQL_STMT := SQL_STMT||'  WHERE  B.PID  is null';
...
SQL_STMT := SQL_STMT||'  AND  A.REPORT_SEGMENT NOT IN (''SKIP'') ) T ';
SQL_STMT := SQL_STMT||'  ORDER BY T.COMPLETEDDATE ';

OPEN GC FOR SQL_STMT USING  GV_CURRENT_MINUS, GV_CURRENT_DATE ;
      LOOP
 ...
 SELECT A.SPIPROPERTYID,
        ...
        CASE WHEN A.COMPLETEDDATE IS NOT NULL THEN A.COMPLETEDDATE + M.MO(I) END COMPLETEDDATE_30,
         ...
         Where a.contractorcode in ('P5GRS','ABRREO','CALRGR','NAMGR2','PSEGRS','BMVREO','RJRRGR','JESRGR')
           AND A.COMPLETEDDATE BETWEEN  TO_DATE('01/01/2017','MM/DD/YYYY') AND TO_DATE('01/31/2017','MM/DD/YYYY');
 commit;
...
SELECT  A.LOANNUMBER,
 ...
 CASE WHEN Q.BACKLOG IS NOT NULL AND Q.ASSIGNMENTS IS NOT NULL  THEN  Q.ASSIGNMENTS
      when R.RECONVEY is not null and data_source NOT in ('CARDS') THEN  'Assign - Reconvey INTICC Exception'
...
SELECT LTRIM(LOANNUMBER,'0') AS LOANNUMBER, CLIENTCODE,  'CARDS' AS DATA_SOURCE
 FROM EIM_PROPERTY_CONVEY_CONDITION
  ))R
 WHERE R.RK = 1) A
------ claims
...
------ latest fail
 ...
 LEFT JOIN ( SELECT U.LOANNUMBER, U.CLIENTCODE, 'RECONVEY' AS RECONVEY
             FROM (SELECT LTRIM(LOANNUMBER,'0') as LOANNUMBER,CLIENTCODE
                    ...
                    UNION
                    SELECT LTRIM(LOANNUMBER,'0') as LOANNUMBER,CLIENTCODE
                     FROM EIM_LOAN_RECONVEY )U  ) R ON ( R.LOANNUMBER = A.LOANNUMBER AND R.CLIENTCODE = A.CLIENTCODE)
 LEFT JOIN ( SELECT LTRIM(LOANNUMBER,'0') AS LOANNUMBER, CLIENT,  REVIEWER AS BACKLOG,   ASSIGNMENTS  FROM SQA_ICC_BACKLOG  WHERE COMPLETED = 0 ) Q ON ( Q.LOANNUMBER = A.LOANNUMBER AND A.CLIENTCODE = Q.CLIENT);
       ...
       COMMIT;
         ...
         COMMIT;
                ...
                insert into SQA_ICC_PRIOR_LOAN_HISTORY ( PID,LOAN_NBR,CLIENT_CODE,OUTCOME,ICC_DECISION, CASIS_ALL_ICC, DATE_UPLOADED, DATE_TO_REVIEW,ACTUAL_REVIEW_DATE,DISPUTE,NBR_IMP,NBR_REVERSALS)
                select  B.PID, b.loannumber,b.client, a.assignment, 'N/A', 'N/A', TRUNC(B.DATE_UPLOADED), TRUNC(B.DATE_TO_REVIEW), TRUNC(B.DATE_TO_REVIEW), 'FALSE',0,0
                from SQA_ICC_assignment_template A
...
------- if NOT REVIEWED don't cary REVIEWER'S NAME
...
END IF;
 ...
 OPEN C1(PAPP_ID, PPAGE_ID, PAPPL_USER, PREPORT_NAME);
      ...
      CLOSE C1;
       ...
       SQL_STMT := ' SELECT LOANNUMBER, BACKLOG, CLIENTCODE, ASSIGNMENT,  ROWID  FROM  SQA_ICC_ASSIGNMENT_TEMPLATE '||WHERE_STMT;
...
OPEN GC FOR SQL_STMT;
        FETCH GC BULK COLLECT INTO cat.LOAN_NUMBER, cat.backlog, cat.CLIENTCODE,cat.ASSIGNMENT, cat.ROWIDS LIMIT RECORDCNT;
CLOSE GC;
...
LEFT JOIN ( SELECT  CL.WORKCODEDESC, CL.LOANNUMBER, CL.CLIENTCODE, CL.ORDERDATE
            FROM ( SELECT  WORKCODEDESC, LOANNUMBER, CLIENTCODE, ORDERDATE
                     FROM OPEN_FHA_CLAIM_VW ORDER BY ORDERDATE DESC ) CL
...
RECORDCNT_FROM := CASE WHEN UPPER(NVL(PLIMIT,'ALL')) = 'ALL' THEN 100000
 ...
 WHERE_FROM_STMT := ' WHERE COMPLETED = 0 AND REVIEWER IN '||IN_FROM_STMT||' ';
         ...
         LOOP
              ...
              EXIT WHEN C1%NOTFOUND;
               ...
               CASE_STMT := CASE  WHEN UPPER(R1.CONDITION_OPERATOR) IN ('IN') THEN R1.CONDITION_COLUMN_NAME||' IN  ('''||REPLACE(R1.CONDITION_EXPRESSION,',',''',''')||''') '
                                  ...
                                  ELSE REPLACE( REPLACE( REPLACE(R1.CONDITION_SQL,'"','') ,'#',''''),'APXWS_EXPR',R1.CONDITION_EXPRESSION)
                             END;
                      WHERE_FROM_STMT :=  WHERE_FROM_STMT||' AND '||CASE_STMT||' AND ';
...
---------- pull the data to reassign
             ...
             SQL_STMT_FROM := ' SELECT LOANNUMBER, CLIENT, REVIEWER, CAR_PROCESSOR, DATE_UPLOADED, DATE_TO_REVIEW, PICK_ORDER, ASSIGNMENTS, ROWID FROM SQA_ICC_BACKLOG '||WHERE_FROM_STMT;
...
--      COMMIT;
      ...
      OPEN GC FOR SQL_STMT_FROM;
          FETCH GC BULK COLLECT INTO FL.LOANNUMBER, FL.CLIENT, FL.REVIEWER, FL.CAR_PROCESSOR, FL.DATE_UPLOADED, FL.DATE_TO_REVIEW, FL.PICK_ORDER,FL.ASSIGNMENTS, FL.ROWIDS LIMIT RECORDCNT_FROM;
      CLOSE GC;
...
END IF;
  ...
  CLOSE C1;
        ...
        ------- match re-assignment
        SQL_STMT := ' SELECT  A.LOANNUMBER, A.CLIENT,  B.BACKLOG,  A.CAR_PROCESSOR, A.DATE_UPLOADED, A.DATE_TO_REVIEW,  A.PICK_ORDER, A.ASSIGNMENTS, A.ROWID FROM SQA_ICC_BACKLOG A';
        SQL_STMT := SQL_STMT||'  LEFT JOIN (SELECT LOANNUMBER, BACKLOG, CLIENTCODE, ASSIGNMENT  FROM  SQA_ICC_ASSIGNMENT_TEMPLATE '||WHERE_STMT||' ) B ON (A.LOANNUMBER = B.LOANNUMBER AND A.CLIENT = B.CLIENTCODE) ';
...
--        COMMIT;
        ...
        OPEN GC FOR SQL_STMT;
                FETCH GC BULK COLLECT INTO bt.LOANNUMBER, bt.CLIENT, bt.REVIEWER, bt.CAR_PROCESSOR, bt.DATE_UPLOADED, bt.DATE_TO_REVIEW,  bt.PICK_ORDER,bt.ASSIGNMENTS, bt.ROWIDS;
        CLOSE GC;

        for y in 1..bt.loannumber.count loop
              execute immediate ' UPDATE SQA_ICC_BACKLOG SET REVIEWER = :A, DATE_TO_REVIEW  = :B  WHERE ROWID = :D ' USING bt.reviewer(y),  TRUNC(SYSDATE), bt.rowids(y);
        end loop;
...
*/
         -------- non match backlog does not contain tamplate record
         --
        SQL_STMT := ' SELECT A.LOANNUMBER, A.CLIENTCODE, A.BACKLOG, A.DATA_SOURCE, A.REPORT_DATE, A.REPORT_DATE, 0 AS PICK_ORDER, A.ASSIGNMENT, A.ROWID  FROM  SQA_ICC_ASSIGNMENT_TEMPLATE A';
...
--        COMMIT;
        ...
        for x in 1..bt.loannumber.count loop
              BEGIN
                      ...
                      INSERT INTO SQA_ICC_BACKLOG ( LOANNUMBER, CLIENT, REVIEWER, CAR_PROCESSOR, DATE_UPLOADED, DATE_TO_REVIEW, PICK_ORDER, COMPLETED, ASSIGNMENTS)
                       VALUES (bt.LOANNUMBER(x), bt.CLIENT(x), bt.REVIEWER(x), bt.CAR_PROCESSOR(x), TRUNC(SYSDATE) , TRUNC(SYSDATE),  bt.PICK_ORDER(x), 0,bt.ASSIGNMENTS(x));
                       COMMIT;
              EXCEPTION
                     WHEN OTHERS THEN
                     NULL;
...
UPDATE SQA_ICC_PRIOR_LOAN_HISTORY
SET  REVIEWED_BY = P638_REVIEWED_BY,
         OUTCOME = P638_DID_QC_PASS,
        COMMENTS = p638_COMMENTS,
    ICC_DECISION = P638_DID_QC_PASS,
    ACTUAL_REVIEW_DATE = TRUNC(SYSDATE),
    NBR_IMP      = PNBR_IMP
WHERE PID   = p638_PID;
...
SQLROWCOUNT := SQL%ROWCOUNT;
    ...
    VALUES ( 'SQA_LOVS', 'QAP_ICC_REVIEW_CORRECTIONS',SYSDATE, SQLROWCOUNT, 'UPDATE HISTORY');
    COMMIT;


    UPDATE SQA_ICC_BACKLOG
       SET DATE_TO_REVIEW = TRUNC(SYSDATE),
           REVIEWER       = p638_REVIEWED_BY 
     WHERE PID = P638_PID;
    ...
    VALUES ( 'SQA_LOVS', 'QAP_ICC_REVIEW_CORRECTIONS',SYSDATE, SQLROWCOUNT, 'CLOSED BACKLOG');
    COMMIT;
...
EXCEPTION
    ...
    VALUES ( 'SQA_LOVS', 'QAP_ICC_REVIEW_CORRECTIONS',SYSDATE, RCODE, MSG);
    COMMIT;
...
BEGIN
    ...
    SELECT TO_NUMBER(VARIABLE_VALUE)
     INTO  CUTOFF_DAYS
     FROM  SQA_SYS_VARIABLES
     WHERE VARIABLE_NAME = 'CUTOFF_DAYS';


        BEGIN 

              EXECUTE IMMEDIATE 'DROP INDEX SQA_TD_DATA_ETL_CMPT ';
        EXCEPTION 
             WHEN OTHERS THEN
        ...
        EXECUTE IMMEDIATE 'DROP INDEX SQA_TD_DATA_ETL_CONTR';
...
CNT := 0;
DEL_STMT := 'DELETE SQA_TD_DATA_ETL WHERE ROWID = :B';

SQL_STMT := 'SELECT PID, ROWID';
SQL_STMT := SQL_STMT||' FROM SQA_TD_DATA_ETL ';
SQL_STMT := SQL_STMT||' WHERE  COMPLETEDDATE <  :A ';

OPEN GC FOR SQL_STMT USING (TRUNC(SYSDATE) - CUTOFF_DAYS) ;
      LOOP
            ...
            EXIT WHEN R.PID.count = 0;
               ...
               FORall k in 1..R.PID.COUNT
                    EXECUTE IMMEDIATE DEL_STMT USING R.ROWIDS(k);                                  
             ...
             VALUES ( 'SQA_LOVS', 'SQA_TD_DATA_ETL',SYSDATE, CNT, 'Removing DATA');
             COMMIT;
...
CLOSE GC;
    ...
    BEGIN 
          ...
          EXECUTE IMMEDIATE 'CREATE INDEX SQA_TD_DATA_ETL_CMPT ON SQA_TD_DATA_ETL (COMPLETEDDATE)';
    EXCEPTION 
         WHEN OTHERS THEN
    ...
    EXECUTE IMMEDIATE 'CREATE INDEX SQA_TD_DATA_ETL_CONTR ON SQA_TD_DATA_ETL (CONTRACTORCODE)';
    ...
    END;
         ...
         VALUES ( 'SQA_LOVS', 'SQA_TD_DATA_ETL',SYSDATE, CNT, 'Complete');
         COMMIT;
